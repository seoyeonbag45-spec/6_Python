...
def load_csv(dedup=True):
    ...
    #sort_values : 제시한 컬럼 기준 정렬 --> 인덱스가 섞일 수 있음
    #reset_index(drop=True) : 인덱스 다시 0,1,2 로 저장해줌
...
def load_companies(raw=False):
    """
        ...
        raw=False 일때는 companies.csv 파일 읽어서 DF 반환

        companies.csv => 정제본. 결츨 0
        raw-companies.csv => 오염본 존재
    """ 
    if raw:
