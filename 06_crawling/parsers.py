...
def get_attr(node, selector, attr, default=""):
    """
        HTML 태그 안에서 '속성값(href, src 등)'을 안젆사게 추출하는 함수
...
def get_number(node, selector, default=0):
    ...
    numbers = re.sub(r"[^\d]", "", text) #숫자 아닌 문자 모두 제거
...
def parse_stocks(html):
    """
        주식 목록 전체 HTML 문자열 통해
        필요한 데이터만 추출해 딕셔너리 리스트로 반환해주는 함수

        동작 순서
        1. 텍스트 형태의 HTML 을 BeautifulSoup 을 이용해 DOM 구조로 변환
